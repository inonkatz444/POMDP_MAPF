...
public class PerseusValueIteration extends ValueIteration{
	...
	public void valueIteration( int cMaxSteps, double dEpsilon, double dTargetValue ){
		...
		System.out.println( "Starting " + getName() + " with " + vBeliefPoints.size() + " belief points " + " |BS| " + m_pPOMDP.getBeliefStateFactory().getBeliefStateCount() );
...
//						" filtered ADR " + round(pComputedADRs.second(), 3 ) +
	...
	protected double executeIteration( int iIteration, double dEpsilon ){
		...
		while( !iterationComplete() ){ 
			...
			if( avMaxAlpha != null )
				...
				dNextValue = MIN_INF;
			//System.out.println( "BeliefPoint = " + bsCurrent + " old value = " + dValue +
			...
			if( dValue > dNextValue ){
				...
				//System.out.println( "backup value = " + dNextValue );
				if( dNextValue < dValue + m_dEpsilon ){
					avNext = getMaxAlpha( bsCurrent );
					//System.out.println( "Choose previous best" );
				}
				else{
					if( dNextValue - dValue > dMaxDelta ){
						dMaxDelta = dNextValue - dValue;
					}
					//System.out.println( "Improving point found: " + bsCurrent + " delta = " + ( dNextValue - dValue ) );
				}
				avMaxAlpha = avNext;
	...
	protected double singleValueFunctionIteration( int iIteration, double dEpsilon, POMDP pomdp ){
		...
		//System.out.println( "Time in init - " + ( iEnd - iStart ) / 1000.0 );
...
//				}
			...
			/*
			if( m_cBackups % 25 == 0 ){
				lEnd = JProf.getCurrentThreadCpuTimeSafe();
				checkADRConvergence( pomdp, 1000.0, pComputedADRs );
				
				m_cExecutionTime += ( lEnd - lStart );
				
				System.out.println( "InnerIteration " + m_cIterations +
						" |Vn| = " + m_vValueFunction.size() +
						" simulated ADR " + ((Number) pComputedADRs.m_first).doubleValue() +
						" filtered ADR " + round( ((Number) pComputedADRs.m_second).doubleValue(), 3 ) +
						" CPU time " + m_cExecutionTime / 1000000000 +
						" #backups " + m_cBackups + 
						" V changes " + m_vValueFunction.getChangesCount() +
						" max delta " + round( dMaxDelta, 3 ) +
						"" );
				...
				lStart = JProf.getCurrentThreadCpuTimeSafe();
			}
			*/
		}

		//System.out.println( "Time in prune - " + cPruneTime / 1000.0 );
		//System.out.println( "Time in backup - " + cBackupTime / 1000.0 );
		//System.out.println( "Checked belief states - " + cChecked + " successful " + cSuccessful );
	...
	protected void prunePoints( AlphaVector avNext ){
		...
		//System.out.println( "Pruned " + cPruned + " points, remaining " + m_vIterationBeliefPoints.size() );
	}
	...
	protected void initIterationPoints() {
		...
		//System.out.println( "Begin setting computed values" );
		...
		//System.out.println( "Done setting computed values" );
