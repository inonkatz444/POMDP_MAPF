...
public abstract class ValueIteration extends PolicyStrategy{
	...
	protected static int g_cTrials = 300;
	protected static int g_cStepsPerTrial = 30;
	...
	protected void computeStepsPerTrial(){
		...
		for( cSteps = 100 ; cSteps < 10000 ; cSteps++ ){
			...
			if( dTailSum <= 0.005 ){
				System.out.println( "#Steps per trials = " + cSteps );
	...
	protected AlphaVector G( int iAction, BeliefState bs, LinearValueFunctionApproximation vValueFunction ){
		...
		if( diff( dSumProbs, 1.0 ) > 0.01 ){
			//Logger.getInstance().log( "VI", 0, "G", cG + ") Sum of probabilities is " + dSumProbs );
			//G( iAction, bs, vValueFunction );
		}
	...
	protected AlphaVector backup( BeliefState bs, LinearValueFunctionApproximation vValueFunction ){
		...
		if( ExecutionProperties.getReportOperationTime() )
		{
			...
			/*
			if( m_cBackups % 100 == 0 ){
				Logger.getInstance().log( "ValueIteration", 0, "backup", "After " + m_cBackups + " backups, avg time for backup " + m_cTimeInBackup / m_cBackups );
				//Logger.getInstance().log( "ValueIteration", 0, "backup", "Total backup time: " + m_cTimeInBackup + ", " + "last measurement " + lTimeAfter );
				Logger.getInstance().log( "ValueIteration", 0, "backup", "Average alpha vector size " + m_cAlphaVectorNodes / m_cBackups );
			}
			*/
		}
		return avResult;
	...
	protected AlphaVector backupTauBased( BeliefState bs, LinearValueFunctionApproximation vValueFunction, boolean bMultiThread ){
		...
		//System.out.println( m_cBackups + ") Backup for belief point " + bs );
		...
		if( bMultiThread ){
			...
			for( iAction = 0 ; iAction < m_cActions ; iAction++ ){
				...
				//aThreads[iAction] = new Thread( aFinders[iAction] );
				//aThreads[iAction].start();
			}
		...
		for( iAction = 0 ; iAction < m_cActions ; iAction++ ){
			...
			//System.out.println( m_cBackups + ") backup: Action value, a = " + iAction + " v = " + dValue );
...
/*	
		double dVal = avMax.dotProduct( bs );
		if( diff( dVal, dMaxValue ) > 0.00001 )
	...
	*/	
	...
	private AlphaVector G( int iAction, LinearValueFunctionApproximation vValueFunction, AlphaVector[] aNext, boolean bMultiThread ) {
		...
		for( iObservation = 0 ; iObservation < m_cObservations ; iObservation++ ){
			if( bMultiThread ){
				...
				avAlpha = aNext[iObservation];
				avG = avAlpha.G( iAction, iObservation );
				//System.out.println( avAlpha + ", " + avG );
			}
		...
		//System.out.println( avSum + ", " + avResult );
	...
	private double findMaxAlphas( int iAction, BeliefState bs, LinearValueFunctionApproximation vValueFunction, AlphaVector[] aNext ) {
		...
		for( iObservation = 0 ; iObservation < m_cObservations ; iObservation++ ){
			...
			if( dProb > 0.0 ){
				...
				/*
				System.out.println( "successor[" + iAction + "," + iObservation + "] = " + bsSuccessor );
				System.out.println( "alpha[" + iAction + "," + iObservation + "] = " + avAlpha );
				System.out.println( "alpha(b) = " + dValue );
				*/
			}
			else{
			...
			//System.out.println( "next[" + iAction + "," + iObservation + "] = " + avAlpha );
		}
		...
		//System.out.println( "R(b," + iAction + ") = " + m_pPOMDP.immediateReward( bs, iAction ) );
	...
	protected AlphaVector backupGBased( BeliefState bs, LinearValueFunctionApproximation vValueFunction ){
		...
		if( ExecutionProperties.useMultiThread() ){
			...
			for( int iAction : m_pPOMDP.getRelevantActions( bs ) ){
				...
				//System.out.println( m_cBackups + ") backup: Action value, a = " + iAction + " v = " + dValue + " " + avCurrent );
	...
	protected double getMaxMinR(){
		/*
		double dMinValue = Double.MAX_VALUE, dMaxValue = MIN_INF, dValue = 0.0;
		...
		for( iAction = 0 ; iAction < m_cActions ; iAction++ ){
			...
			for( iStartState = 0 ; iStartState < m_cStates ; iStartState++ ){
				for( iEndState = 0 ; iEndState < m_cStates ; iEndState++ ){
					dValue = m_pPOMDP.R( iStartState, iAction, iEndState );
					if( dValue < dMinValue ){
						dMinValue = dValue;
					}
				}
	...
	protected void initValueFunctionToMin(){
		...
		AlphaVector avMin = m_vValueFunction.getFirst();
		//m_avMaxValues = avMin.copy();		
	}
	...
	protected void initValueFunctionToMin( LinearValueFunctionApproximation vValueFunction ){
		...
		System.out.println( "Min R value = " + dMinValue + " init value = " + dDefaultValue );
...
/*
		AlphaVector avMin = new SparseAlphaVector( null, m_cStates, m_cActions, m_cObservations, 
				m_fTransition, m_fObservation, 0 );
				*/
	...
	protected void initValueFunctionUsingBlindPolicy( LinearValueFunctionApproximation vValueFunction ){
		...
		if( m_sBlindPolicyValueFunctionFileName != null ){
			try{
				...
				System.out.println( "Could not load blind policy - " + e );
			}
		}
		...
		for( iAction = 0 ; iAction < m_cActions ; iAction++ ){
			...
			while( dMaxResidual > 0.1 ){
				...
				for( iState = 0 ; iState < m_cStates ; iState++ ){
					...
					double dSumTr = 0.0;
					for( iEndState = 0 ; iEndState < m_cStates ; iEndState++ ){
						dTr = m_pPOMDP.tr( iState, iAction, iEndState );
						if( dTr > 0 ){
							dSumTr += dTr;
							dValue = av.valueAt( iEndState );
							dSum += dTr * dValue;
					...
					if( dSumTr > 1.5 ){
						System.out.println("BUGBUG " +dSumTr);
						for( iEndState = 0 ; iEndState < m_cStates ; iEndState++ ){
							dTr = m_pPOMDP.tr( iState, iAction, iEndState );
							System.out.println( "tr( " + iState + " , "  + iAction + ", " + iEndState + " ) = " + dTr );
						}
					}
					*/
					...
					//if( iState == 15 )
					//	System.out.println( "state " + iState + ") " + av.valueAt( iState ) + " -> " + dNewValue + " = " + dDiff + " max " + dMaxDiff );
				}
				...
				//System.out.println( iIteration + ") diff = " + dMaxDiff + " state " + iMaxDiffState + ", " + av  );
			...
			//System.out.println();
			System.out.println( "Done action " + iAction +
		...
		if( m_sBlindPolicyValueFunctionFileName != null ){
			try{
				...
				System.out.println( "Could not save blind policy - " + e );
			}
		}
	...
	protected void initValueFunctionUsingBlindPolicy( int cMaxIterations, LinearValueFunctionApproximation vValueFunction ){
		...
		for( iIteration = 0 ; iIteration < cMaxIterations ; iIteration++ ){
			...
			for( iAction = 0 ; iAction < m_cActions ; iAction++ ){
				...
				/*
				avNext = new TabularAlphaVector( null, m_cStates, m_cActions, m_cObservations, 
						m_fTransition, m_fObservation, 0 );*/
				if( iIteration > 0 )
			...
			//System.out.println( "After " + iIteration + " iterations: V = " + toString( m_vValueFunction ) );
		}
		//System.out.println( "Done blind policy: " + vValueFunction );
	}
	...
	protected Object choose( Vector vObjects ){
		int iObject = m_rndGenerator.nextInt( vObjects.size() );
		//iObject = vObjects.size() - 1;
	...
	public double computeBellmanError( BeliefState bsCurrent, LinearValueFunctionApproximation vValueFunction ){
		...
		else{ //Idiotic algorithm - will never work. It can be that the maximal error was not reduced but another action can be improved by more.
			iAction = -1;
	...
	/*
		...
		*/
		...
		pComputedADRs.setFirst(0.0);
		pComputedADRs.setSecond(0.0);
		...
		if( pomdp != null && g_cTrials > 0 ){
			...
			//if( dSimulatedADR > 0.0 && m_vValueFunction.size() > 50 )
			...
			if( pComputedADRs != null ){
				pComputedADRs.setFirst(dSimulatedADR);
				pComputedADRs.setSecond(m_dFilteredADR);
			}						
		}
		
		if( bIndependentBeliefState ){
			System.out.println( "Factored operations - Tau: - operations " + FactoredBeliefState.getTauComputationCount() + " avg time " + 
