...
public abstract class LDistance implements DistanceMetric {
	...
	protected double getCachedDistance( BeliefState bs1, BeliefState bs2 ){
		try{
			TreeMap tmSingle = (TreeMap) m_tmCache.get(bs1.getId());
			if( tmSingle != null ){
				Double dValue = (Double) tmSingle.get(bs2.getId());
				if( dValue != null )
	...
	protected void cacheDistance( BeliefState bs1, BeliefState bs2, double dDistance ){
		TreeMap tmSingle1 = (TreeMap) m_tmCache.get(bs1.getId());
		TreeMap tmSingle2 = (TreeMap) m_tmCache.get(bs2.getId());
		if( tmSingle1 == null ){
