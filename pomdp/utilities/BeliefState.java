...
public abstract class BeliefState extends PriorityQueueElement implements Serializable{
	...
	protected int m_iID;
	private static double m_dEpsilon = BeliefStateFactory.m_dEpsilon;
	...
	public BeliefState( int cStates, int cActions, int cObservations, int id, boolean bCacheBelifStates, BeliefStateFactory bsFactory ){
		super();
		...
		m_vPredecessors = new Vector();
		m_vNeighbors = new Vector();
		m_cVisits = 0;
		...
		m_dComputedValue = 0;
		m_mProbCurrentGivenPred = new TreeMap( getComparator() );
	...
	public synchronized void addSuccessor( int iAction, int iObservation, BeliefState bsSuccessor ){
		if( m_amSuccessors == null )
			return;
		Integer iKey = iObservation;
		double dProb = probabilityOGivenA( iAction, iObservation );
		Pair pEntry = new Pair( bsSuccessor, dProb);
	...
	public BeliefState nextBeliefState( int iAction, int iObservation ){
		...
		if( m_bCacheBelifStates && getBeliefStateFactory().isCachingBeliefStates() ){
			Integer iKey = iObservation;
	...
	public Iterator getSortedSuccessors( int iAction ){
		if( m_sSortedSuccessors == null )
			m_sSortedSuccessors = new SortedSet[m_cActions];
		if( m_sSortedSuccessors[iAction] == null )
			m_sSortedSuccessors[iAction] = new TreeSet( new ReversePairComparator() );
	...
	public synchronized void addPredecessor( BeliefState bs, double dProb, int iAction ){
		if( m_bCacheBelifStates && !m_vPredecessors.contains( bs ) ){
			...
			m_mProbCurrentGivenPred.put( bs, new Pair(dProb, iAction) );
		}
	}
	...
	protected Element getPreds( Document docBeliefSpace ){
		...
		Iterator itPreds = m_vPredecessors.iterator();
	...
	public Pair getCurrentGivenPredecessorDetails( BeliefState bsPredecessor ){
		return (Pair) m_mProbCurrentGivenPred.get( bsPredecessor );
	}
	
	public double getProbCurrentGivenPredecessor( BeliefState bsPredecessor ){
		Pair p = getCurrentGivenPredecessorDetails( bsPredecessor );
		if( p == null )
			return 0.0;
		Double dProb = (Double)p.m_first;
		if( dProb == null )
			return 0.0;
	...
	public int getActionCurrentGivenPredecessor( BeliefState bsPredecessor ){
		...
		if( p == null )
			return -1;
		Integer iAction = (Integer)p.m_second;
		if( iAction == null )
			return -1;
		...
		for( iAction = 0 ;iAction < m_cActions ; iAction++ ){
			if( m_amSuccessors[iAction] != null ){
				...
				while( itSuccessors.hasNext() ){
					...
					if( bsSuccessor == null )
						System.out.println( getId() + "," + iAction + "," + e );
	...
	public void computeAllSuccessors( int iAction ){
		...
		for( iObservation = 0 ; iObservation < m_cObservations ; iObservation++ ){
			...
			if( dProb > 0.0 ){
				...
				pEntry = new Pair( bsNext, dProb);
				cEntries++;
				m_sSortedSuccessors[iAction].add( pEntry );
				addSuccessor( iAction, iObservation, bsNext );
				if( m_sSortedSuccessors[iAction].size() != cEntries )
					System.out.println( "Real entries " + m_sSortedSuccessors[iAction].size() + ", intended " + cEntries );
				dSumProbs += dProb;
			}
		}
	}
