...
public class Table 
	...
	_hmTableMap.put(new ADDRNode(new_id), 
			new T((T)src._hmTableMap.get(new ADDRNode(id))));
	return new_id;
    }
	...
	vars.add(_hmGVarToLevel.get(gid));
	T table = new T(vars);
	ArrayList setting = new ArrayList();
	setting.add(Boolean.FALSE);
	table.set(setting, low);
	setting.set(0, Boolean.TRUE);
	...
	if (gid != -1) {
		...
		System.err.println("Table.T.copyFrom: GID index not found for restriction");
	...
	for (int c = 0; c < table._entries; c++) {
		...
		Boolean bset = (((c >> (table._sz - b - 1)) & 1) == 1) ? TRUE : FALSE;
		table._settings.set(b, bset);
		eval_var_setting.set( ((Integer)table._alVars.get(b)).intValue(), 
				 bset );
	    }
	...
	case ARITH_SUM: {
		...
		for (int j = 0; j < nvars; j++) {
		    settings.set(j, (((i >> (nvars - j - 1)) & 1) == 1) ? TRUE : FALSE);
		}
		tr.set( settings, 
			( t1.projectAndEval( new_vars, settings ) + 
			  t2.projectAndEval( new_vars, settings )));
	    }
	} break;
	case ARITH_PROD: {
	    for (int i = 0; i < entries; i++) {
	...
	case ARITH_MIN: {
		...
		tr.set( settings, 
			( Math.min(t1.projectAndEval( new_vars, settings ), 
				   t2.projectAndEval( new_vars, settings )) ));
	    }	    
	} break; 
	case ARITH_MAX: {
		...
		tr.set( settings, 
			( Math.max(t1.projectAndEval( new_vars, settings ),
				   t2.projectAndEval( new_vars, settings )) ));
	    }	    
	} break; 
	case ARITH_DIV: {
    ...
    public static ArrayList MergeSortedLists(ArrayList li, 
					     ArrayList lj) {
	...
	public T(ArrayList vars) {
	    ...
	    _entries = 1 << _sz;
	    if (_sz > 30) {
		System.out.println("Table.T: ERROR: Size exceeds available address space");
		System.exit(1);
	    }
	...
	public void projectAndSet(ArrayList supset_vars, 
		...
		int compare_val = ((Integer)_alVars.get(i)).compareTo(
						 (Integer)supset_vars.get(j));
		if (compare_val == 0) {
	...
	public String toString() {
		...
		for (int b = _sz - 1; b >= 0; b--) {
		    _settings.set(b, (((c >> (_sz - b - 1)) & 1) == 1) ? TRUE : FALSE);
		}
		
		// Now show the assignment
		sb.append("Assignment: " + 
			  DD.PrintBitVector(_settings) + " -> " + 
