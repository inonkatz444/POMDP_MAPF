...
public class ADD 
    ...
    public HashMap    _hmPairs;       // <ADD-v1-id, ADD-v2-id>   -> ADDNode (for ADD-3) (Apply cache)
	...
	for (int i = 0; i < _alOrder.size(); i++) {
	    _hmGVarToLevel.put((Integer)_alOrder.get(i), i);
	}	
    }
	...
	if (n instanceof ADDINode) {
	    ...
	    _hmNewINodeCache.put(new SINodeIndex(ni._nGlobalID, ni._nLow, ni._nHigh), r);
	...
	ADDNode n = getNode(id);
	if (n instanceof ADDINode) {
	    gset.add(((ADDINode) n)._nGlobalID);
	...
	//System.out.println("Create: <" + gid + "," + low + "," + high + ">  ->  " + _nLocalIDCnt);
	...
	int      lid = _nLocalIDCnt++;
	ADDDNode n   = new ADDDNode(lid, min_val, max_val, lower_label, upper_label);
	_hmADDNodes.put(new ADDRNode(lid), n);
	...
	ADDRNode iret = (ADDRNode)_hmINodeCache.get(new SINodeIndex(gid, low, high));
	if (iret != null) {
	    return iret._lid;
	} else if (create) {
    ...
    public int getDNode(double min_val, double max_val, boolean create, 
			String lower_label, String upper_label) {
	ADDRNode iret = (ADDRNode)_hmDNodeCache.get(new SDNodeIndex(min_val, max_val));
	if (iret != null) {
	...
	// Get all DNodes and convert to BNodes
	Iterator i = ret_add._hmDNodeCache.entrySet().iterator();
	while (i.hasNext()) {
	    Map.Entry e      = (Map.Entry)i.next();
	    SDNodeIndex dni  = (SDNodeIndex)e.getKey();
	    ADDRNode id       = (ADDRNode)e.getValue();
	    ret_add._hmADDNodes.put(id, new ADDBNode(id.intValue(), dni._dLowerVal == lower && 
						     dni._dUpperVal == upper ));
	}
	ret_add._hmDNodeCache = new HashMap();
	ret_add = new ADD(ret_add);
    ...
    // Get a list of BDDs from an ADD
    public static ArrayList GetBDDsFromADD(ADD orig) {

	ArrayList bdd_list   = new ArrayList();
	ArrayList val_list   = new ArrayList();
	ArrayList label_list = new ArrayList();

	// Build a TreeSet of the distinct terminal values
	TreeSet vals = new TreeSet(new SDNodeIndex.SDNComparator(false)); // => decreasing
	Iterator i = orig._hmDNodeCache.entrySet().iterator();
	while (i.hasNext()) {
	    Map.Entry e = (Map.Entry)i.next();
	    vals.add( e.getKey() );
	    ADDRNode id = (ADDRNode)e.getValue();
	    label_list.add ( ((ADDDNode)orig.getNode(id.intValue()))._sLowerLabel ); // TODO: Both Labels!
	}

	// Now iterate through these nodes, create the BDDs, and add to ret list
	i = vals.iterator();
	while (i.hasNext()) {
	    SDNodeIndex cur = (SDNodeIndex)i.next();
	    val_list.add( cur );
	    bdd_list.add( GetBDDFromADD(orig, cur._dLowerVal, cur._dUpperVal) );
	}

	ArrayList ret_list = new ArrayList();
	ret_list.add(val_list);
	ret_list.add(bdd_list);
	ret_list.add(label_list);
    ...
    **************/
    ...
    // TODO: Make this work with ADDSNode... only used for FOMDP epsilon convergence
	...
	while (i.hasNext()) {
	    ...
	    double upper = (dni._dUpperVal < 0) ? -dni._dUpperVal : dni._dUpperVal;
	    double lower = (dni._dLowerVal < 0) ? -dni._dLowerVal : dni._dLowerVal;
	...
	if (prevTrue  > INVALID)  { ((ADDBNode)getNode(prevTrue))._bVal  = false; }
	if (prevFalse > INVALID)  { ((ADDBNode)getNode(prevFalse))._bVal = true;  }
	...
	if (range <= PRUNE_PRECISION) {
		...
		//    return getDNode(ni._dMaxUpper, ni._dMaxUpper, true); 
		//} else {
		//    //  System.out.println("Illegal replacement type " + PRUNE_TYPE); // + ", " + _nWhich
	...
	if ( (retc = (ADDRNode)_hmPruneMap.get(_tmpADDRNode)) == null ) {
	    ...
	    if (USE_QUICK_PRUNE) {
		max_abs_diff = Math.max(getMax(high) - getMin(low),
					getMax(low)  - getMin(high));
	    } else {
		ADDNode adiff = getNode(applyInt(low, high, ARITH_MINUS));
		if (adiff instanceof ADDINode) {
		    ADDINode adi = (ADDINode)adiff;
		    max_abs_diff = Math.max(Math.abs(adi._dMinLower), 
	    ...
	    //	System.out.println("Difference: " + max_abs_diff + " / " + max_abs_diff2);
	...
	if (n instanceof ADDBNode) {
	    ...
	    ADDDNode dn = (ADDDNode)n;
	    ret = getDNode(dn._dLower, dn._dUpper, true, dn._sLowerLabel, dn._sUpperLabel);
		...
		if (op == RESTRICT_HIGH || op == RESTRICT_LOW) {
		    ret = ((op == RESTRICT_LOW) ? 
			   reduceRestrict(ni._nLow, src, gid, op) :
			   reduceRestrict(ni._nHigh, src, gid, op));
		} else {
		...
		//System.out.println("Get:    <" + ni._nGlobalID + "," + low + "," + high + ">");
		ret = getINode(ni._nGlobalID, low, high, true);
		//System.out.println("Result: <" + ni._nGlobalID + "," + low + "," + high + ">  ->  " + ret);
	...
	//if ((id != ret) && (src == this)) {
	//    //System.out.println("Alternate reduction of " + id + "(del) to " + ret + "(new)");
	...
	result.setRoot(result.apply(((ADD)a1)._nRoot, ((ADD)a1), ((ADD)a2)._nRoot, ((ADD)a2), op));
	...
	if (t > INVALID) {
		...
		v1low = n1i._nLow; v1high = n1i._nHigh;
	    } else {
		v1low = a1;        v1high = a1;
	    }
		...
		v2low = n2i._nLow; v2high = n2i._nHigh;
	    } else {
		v2low = a2;        v2high = a2;
	    }
	...
	case ARITH_MIN: { // Can assume an ADD for arithmetic operations
		...
		ret = getDNode(lower, upper, true, 
			       (lower == a1d._dLower) ? a1d._sLowerLabel : a2d._sLowerLabel,
			       (upper == a1d._dUpper) ? a1d._sUpperLabel : a2d._sUpperLabel );
		...
		ADDDNode a1d = (ADDDNode)a1;
		ret = getDNode( a1d._dLower, a1d._dUpper, true, a1d._sLowerLabel, a1d._sUpperLabel );
		...
		ADDDNode a2d = (ADDDNode)a2;
		ret = getDNode( a2d._dLower, a2d._dUpper, true, a2d._sLowerLabel, a2d._sUpperLabel );
		//System.out.println("MIN PRUNE!!!"); // TODO: Remove!!!
		MIN_PRUNE_CNT++;
	    } 
	} break;
	...
	while ( cur instanceof ADDINode ) {
		...
		cur = (b.booleanValue()) ? getNode(ni._nHigh) : getNode(ni._nLow);
	    } else {
		return Double.NaN;
	    }
	}
	...
	int l1 = ((Integer)_hmGVarToLevel.get(gid1)).intValue();
	int l2 = ((Integer)_hmGVarToLevel.get(gid2)).intValue();
	...
	if (o instanceof String && HasOnlyDigits((String)o)) {
	    ...
	    high_br = applyInt(high_br, 
			       buildDDFromUnorderedTree((ArrayList)l.get(1), var2ID) /*high*/, 
			       ARITH_PROD);

	    // Get the !var ADD
	    int low_br = getVarNode(gid, 1.0d, 0.0d);
	    low_br = applyInt(low_br, 
			      buildDDFromUnorderedTree((ArrayList)l.get(2), var2ID) /*low*/,
			      ARITH_PROD);
    ...
    public void dumpToTree(int n, String var_prefix, PrintWriter ps, DecimalFormat df, int level) {
