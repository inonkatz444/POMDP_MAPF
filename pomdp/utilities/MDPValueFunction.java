...
//		}
		...
		dDelta = diff( dMaxQValue, getValue( iStartState ) /*m_adValues[iStartState]*/ );
	...
	*/
			...
			*/
			...
			if( m_ivBestActions != null ){
				for( int iState : getValidStates() ){
					...
					//System.out.println( iState + ", " + getStateName( iState ) + " => " + iAction );
	...
	protected void improveVectors( int cMaxIterations ){
		...
		for( iIteration = 0 ;iIteration < cMaxIterations ; iIteration++ ){
			...
			System.out.println( "After " + iIteration + " iterations: V = " + m_vValueFunction );
		}
	}
...
//			Logger.getInstance().logFull( "MDPVI", 0, "ComputeV", "Start iteration " + iIteration );
			for( int iState : getValidStates() ){
				...
				if( m_adValues.elementAt( iState ) == 0.0 ){
					System.out.println( "V(" + iState + ") = " + m_adValues.elementAt( iState ) );
					for( iAction = 0 ; iAction < m_cActions ; iAction++ ){
...
//						" max " + iMaxState + " = " + dMaxValue );
		...
		for( int j = 0 ; j < m_cStates ; j++ ){
			...
			int iMax = -1;
			for( int i = 0 ; i < m_cStates ; i++ ){
				if( tr( j, iMaxAction, i ) > 0.0 ){
					if( m_adValues.elementAt( i ) > dMax ){
						dMax = m_adValues.elementAt( i );
						iMax = i;
					}
				}
			}
	...
	public synchronized void valueIteration( int cMaxIterations, double dEpsilon ){
		...
		if( PERSIST_FUNCTION && !m_bLoaded && !sFileName.equals( "" ) ){
			...
			return; 
				...
				System.out.println( "Unable to load QMDP value function: " + e );
				//e.printStackTrace();
			}
			catch( Error e ){
				System.out.println( "Unable to load QMDP value function: " + e );
		...
		if( !m_bLoaded ){
			...
			try{
				...
				System.out.println( "Error in computeVN: " + e );
				throw e;
			}
